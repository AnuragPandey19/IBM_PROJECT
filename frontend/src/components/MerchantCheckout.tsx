...
export type MerchantProduct = {
  id: string;
  name: string;
  priceInr: number;    // Displayed to the user in INR
  image: string;       // Emoji or short symbol
};

export type MerchantConfig = {
  slug: string;                    // 'zomato', 'swiggy', 'bigbasket'
  displayName: string;             // 'Zomato'
  tagline: string;                 // 'Order from your favorite restaurants'
  logoLetter: string;              // 'Z'
  primaryHex: string;              // '#E23744'
  primaryHexDark: string;          // '#C21F30'
  category: string;                // Sparkov merchant category — 'grocery_pos', 'shopping_net', etc.
...
type CartItem = { product: MerchantProduct; qty: number };

// ------------------------------------------------------------------
// Currency
// ------------------------------------------------------------------
// The ML model is trained on USD amounts (Sparkov = US dataset). We display
// INR to make the merchant portals feel realistic for Indian brands, but
// convert to USD when calling the fraud API.
const INR_PER_USD = 80.0;

const inrToUsd = (inr: number) => inr / INR_PER_USD;
...
export function MerchantCheckout({ config }: { config: MerchantConfig }) {
  ...
  useEffect(() => {
    if (config.products.length > 0 && cart.length === 0) {
      setCart([{ product: config.products[0], qty: 1 }]);
    }
  ...
  useEffect(() => {
    ...
    if (p) {
      ...
      if (!cardholderName) {
        ...
        setCardholderName(map[p.label] ?? "");
      }
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedProfile, profiles]);

  const cartTotalInr = cart.reduce((sum, item) => sum + item.product.priceInr * item.qty, 0);
  const cartTotalUsd = inrToUsd(cartTotalInr);

  function addToCart(product: MerchantProduct) {
    setCart((c) => {
  ...
  function updateQty(productId: string, qty: number) {
    ...
    else setCart((c) => c.map((i) => (i.product.id === productId ? { ...i, qty } : i)));
  }
  ...
  async function handlePay() {
    ...
    const body = {
      ...
      card_expiry: expiry,
      card_cvv: cvv,
      amount: cartTotalUsd,          // USD sent to backend model
      merchant_name: config.displayName,
      merchant_category: config.category,
      cust_email: email || undefined,
      company_slug: config.slug,     // Routes txn to merchant's own company_id
  ...
  const gradient = `linear-gradient(135deg, ${config.primaryHex} 0%, ${config.primaryHexDark} 100%)`;

  return (
    <div className="min-h-screen bg-slate-950 text-slate-100">
      {/* Header — branded per merchant */}
      <header className="border-b border-slate-800 bg-slate-950/90 backdrop-blur">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="font-bold text-lg leading-tight">{config.displayName}</div>
          ...
          <div className="flex items-center gap-3 text-xs text-slate-500">
            ...
            <span className="hidden sm:flex items-center gap-1.5 font-semibold" style={{ color: config.primaryHex }}>
              Payments by CHIMERA-FD
            </span>
          </div>
        </div>
      </header>
      ...
      <div className="max-w-6xl mx-auto px-6 py-8 grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-3 space-y-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {config.products.map((p) => (
                <button
                  ...
                  type="button"
                  onClick={() => addToCart(p)}
                  className="bg-slate-900/60 border border-slate-800 rounded-xl p-3 text-left transition group"
                ...
                >
                  ...
                  <div className="text-sm font-mono font-bold mt-1" style={{ color: config.primaryHex }}>
                    {fmtInr(p.priceInr)}
                  </div>
                  <div className="text-[10px] text-slate-500 mt-1 transition">
                    + Add to cart
                  </div>
                </button>
              ))}
          ...
          <section>
            <h2 className="text-sm uppercase tracking-widest font-bold text-slate-400 mb-3">Your cart</h2>
            {cart.length === 0 ? (
              ...
              <div className="bg-slate-900/60 border border-slate-800 rounded-xl overflow-hidden">
                {cart.map((item) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-slate-500 font-mono">{fmtInr(item.product.priceInr)} each</div>
                    </div>
                    ...
                    <div className="text-sm font-mono font-bold w-24 text-right">
                      {fmtInr(item.product.priceInr * item.qty)}
                    </div>
                  </div>
                ))}
                <div className="p-3 flex items-center justify-between bg-slate-950/50">
                  ...
                  <span className="text-xl font-mono font-black" style={{ color: config.primaryHex }}>
                    {fmtInr(cartTotalInr)}
                  </span>
                </div>
              </div>
        ...
        <div className="lg:col-span-2">
          <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5 sticky top-6">
            <h2 className="text-sm uppercase tracking-widest font-bold text-slate-400 mb-4">Payment details</h2>
            ...
            <div className="mt-5 pt-4 border-t border-dashed border-slate-800">
              ...
              <div className="space-y-3">
                ...
                <FormField label="Force hour of day">
                  <select
                    value={demoHour}
            ...
            <button
              ...
              className="mt-5 w-full py-3 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed text-white font-bold text-sm shadow-lg transition"
              style={{
                background: gradient,
                boxShadow: `0 10px 30px -12px ${config.primaryHex}80`,
              }}
            >
              {loading ? (
                ...
                <span>Pay {fmtInr(cartTotalInr)}</span>
              )}
            </button>
      ...
      <footer className="border-t border-slate-800 mt-12">
        <div className="max-w-6xl mx-auto px-6 py-6 text-xs text-slate-500 flex flex-wrap items-center justify-between gap-3">
          <span>Demo storefront using CHIMERA-FD fraud detection API · merchant: {config.displayName}</span>
...
function ResultScreen({
  result, config, cartTotalInr, showDeveloperView, onToggleDev, onReset,
}: {
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex items-start justify-center pt-16 pb-16 px-6">
      <div className="w-full max-w-lg">
        <div className={`rounded-2xl border p-8 text-center ${
          ...
          <div className="text-sm text-slate-400 mt-2 leading-relaxed">{result.decision_reason}</div>
        </div>

        <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-5 mt-4 space-y-2 text-sm">
          <ReceiptRow label="Merchant" value={config.displayName} />
          <ReceiptRow label="Amount" value={fmtInr(cartTotalInr)} bold />
          <ReceiptRow label="Amount (USD sent to fraud engine)" value={`$${result.amount_charged.toFixed(2)}`} small mono />
          ...
          <ReceiptRow label="Processed in" value={`${result.decision_time_ms.toFixed(1)} ms`} small />
        </div>
        ...
        <div className="mt-4 flex items-center justify-between text-xs">
          ...
          <button
            type="button"
            onClick={onReset}
            className="text-slate-300 font-semibold hover:opacity-80"
        ...
        {showDeveloperView && (
          <div className="mt-4 bg-slate-900/60 border border-slate-800 rounded-2xl p-5 font-mono text-xs">
            ...
            <div className="space-y-1 text-slate-300">
              ...
              <div><span className="text-slate-500">company_slug:</span> {config.slug}</div>
            </div>
            ...
            <div className="mt-4 pt-3 border-t border-slate-800 text-[11px] text-slate-400">
              Analyst dashboard: <Link href="/login" className="hover:underline" style={{ color: config.primaryHex }}>log in as {config.displayName} admin</Link> to see this transaction.
            </div>
          </div>
        )}
      </div>
...
const inputCls =
  "w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white placeholder-slate-600 focus:outline-none focus:border-slate-500";
